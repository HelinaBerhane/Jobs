...
class JobsRepository:
    ...
    async def create(self, job: Job) -> Job:
        ...
        await self.database.execute(query, job.dict())
        return job

    async def read_one(self, job_id: UUID) -> Optional[Job]:
        query = "SELECT id, created_date, name FROM jobs WHERE id = :id"
        row = await self.database.fetch_one(query, {"id": job_id})
        return parse_job(row) if row else None

    async def read_many(self, job_ids: Optional[List[UUID]]) -> List[Job]:
        if job_ids:
    ...
    async def delete(self, job_id: UUID) -> None:
        ...
        await self.database.execute(query, {"id": job_id})
